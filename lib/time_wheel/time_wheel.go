...
const (
	defaultTimerTick      = time.Millisecond
	defaultTimerWheelSize = 512
)

var Time = NewTimeWheel(10*defaultTimerTick, defaultTimerWheelSize)
...
func NewTimeWheel(tick time.Duration, wheelSize int64) *TimeWheel {
	...
	if tickMs <= 0 {
		log.Error("tick must be greater than or equal to 1ms")
		return nil
	}
...
func (tw *TimeWheel) AfterFunc(id string, d time.Duration, f func(), async ...bool) *Timer {
	t := &Timer{
		id:         id,
...
// is non-zero.
func (tw *TimeWheel) ScheduleFunc(id string, s Scheduler, f func(), async ...bool) *Timer {
...
func (tw *TimeWheel) NextId() string {

	return uint64ToId(atomic.AddUint64(&tw.seqId, 1))
}
