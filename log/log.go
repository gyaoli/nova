...
func CreateLogger() gen.LoggerBehavior {
	return myLogger
}
...
func (l *Logger) Log(message gen.MessageLog) {
	...
	case gen.LogLevelTrace, gen.LogLevelDebug:
		l.Debug(text)
	case gen.LogLevelWarning:
		l.Warn(text)
	case gen.LogLevelError, gen.LogLevelPanic:
		// Ergo panic is a log severity. Do not call zap.Panic here.
		l.Error(text)
	default:
		l.Info(text)
	}
...
func (l *Logger) Terminate() {
	// The Nova logger is shared by the whole process and is flushed by main.
}
